...
def _displayStatus(mthread,linkm,queue, timeout=10):
    """
    timeout 指定时间输出内容
    ...
    while True:
        ...
        record = linkm.status()
        print( "current task: %s    current thread: %s total record: %s link on this site" % (task,thread,record))

def displayStatus(mthread,linkm,queue):
    t = threading.Thread(target=_displayStatus, args = (mthread,linkm,queue))
    t.start()
